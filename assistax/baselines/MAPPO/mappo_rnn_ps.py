"""
Multi-Agent Proximal Policy Optimization (MAPPO) with RNN and Parameter Sharing

This implementation provides a clean, research-friendly MAPPO algorithm with:
- SHARED actor network parameters across all agents (parameter sharing)
...
Key Difference from No-Parameter-Sharing Version:
- All agents share the same actor network parameters, improving sample efficiency
- Agent observations are concatenated into a single batch dimension
- More memory efficient but agents have identical policies

Based on the JaxMARL Implementation of PPO with Parameter Sharing.
"""
...
class ActorRNN(nn.Module):
    """
    Shared actor network with parameter sharing across all agents.
    
    Key Feature: All agents share the same network parameters, which improves
    sample efficiency but means all agents have identical policies.
    ...
    def __call__(self, hstate, x):
        ...
        Args:
            hstate: Hidden state from previous timestep (concatenated across agents)
            x: Tuple of (observations, done_flags, available_actions)
            
        Returns:
            Tuple of (new_hidden_state, action_distribution)
        """
...
class CriticRNN(nn.Module):
    ...
    In parameter sharing setup, processes concatenated global observations from all agents.
    """
    config: Dict
...
def batchify(qty: Dict[str, jnp.ndarray], agents: Sequence[str]) -> jnp.ndarray:
    """
    Convert dictionary of agent-specific arrays to concatenated batch.
    
    For parameter sharing, we concatenate all agents' data into a single
    batch dimension rather than keeping separate agent dimensions.
    ...
    Returns:
        Concatenated array with all agents' data in single batch dimension
    """
...
def unbatchify(qty: jnp.ndarray, agents: Sequence[str]) -> Dict[str, jnp.ndarray]:
    """
    Convert concatenated batch back to dictionary of agent-specific arrays.
    
    For parameter sharing, we split the concatenated batch back into
    individual agent arrays.
    
    Args:
        qty: Concatenated array with all agents' data
...
def make_train(config, save_train_state=False):
    """
    Create the main training function for MAPPO with parameter sharing.
    ...
    def train(rng, lr, ent_coef, clip_eps):
        ...
        # Note: For parameter sharing, we concatenate all agents into single batch
        init_x_actor = (
            jnp.zeros((1, env.num_agents * config["NUM_ENVS"], config["OBS_DIM"])),    # obs
            jnp.zeros((1, env.num_agents * config["NUM_ENVS"])),                       # done
            jnp.zeros((1, env.num_agents * config["NUM_ENVS"], config["ACT_DIM"])),    # avail_actions
        )
        init_hstate_actor = jnp.zeros(
            (env.num_agents * config["NUM_ENVS"], config["network"]["gru_hidden_dim"])
        )
        init_x_critic = (
            jnp.zeros((1, env.num_agents * config["NUM_ENVS"], config["GOBS_DIM"])),   # global_obs
            jnp.zeros((1, env.num_agents * config["NUM_ENVS"])),                       # done
        )
        ...
        # For parameter sharing, we flatten the agent dimension
        ...
        def _update_step(runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                # Prepare observations for parameter sharing
                obs_batch = batchify(runner_state.last_obs, env.agents)
                
                # Tile global observations for each agent (parameter sharing approach)
                ...
                # Sample actions from shared policy
                rng, act_rng = jax.random.split(rng)
                action, log_prob = pi.sample_and_log_prob(seed=act_rng)
                
                # Remove time dimension and prepare for environment
                ...
                # Process environment outputs for parameter sharing
                done_batch = batchify(done, env.agents)
                all_done = jnp.tile(done["__all__"], env.num_agents)  # Tile for all agents
            ...
            def _update_epoch(update_state, unused):
                ...
                # Create batch with added step dimension for RNN processing
                batch = UpdateBatch(
                ...
                # Reorganize batch: (step, agent*env, ...) -> (agent*env, step, ...)
                ...
                # Final reorganization: (n_mini, minibatch_size, step, ...) -> (n_mini, step, minibatch_size, ...)
                minibatches = jax.tree_util.tree_map(
                    lambda x: x.swapaxes(1, 2), minibatches
                )
...
def make_evaluation(config):
    """
    Create evaluation function for trained MAPPO agents with parameter sharing.
    ...
    def run_evaluation(rng, train_state, log_eval_info=EvalInfoLogConfig()):
        ...
        # Initialize states (flattened for parameter sharing)
        ...
        def _env_step(runner_state, unused):
            ...
            # Process outputs for parameter sharing
            done_batch = batchify(done, env.agents)
...
def main(config):
    """
    Main function to run MAPPO training with parameter sharing.
    ...
    # Run training with optional JIT compilation
